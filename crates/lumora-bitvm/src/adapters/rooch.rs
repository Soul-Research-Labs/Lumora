...
pub struct RoochBridge<T: RpcTransport = OfflineTransport> {
    config: RoochConfig,
    transport: T,
    next_id: Cell<u64>,
}

impl RoochBridge<OfflineTransport> {
    pub fn new(config: RoochConfig) -> Self {
        Self {
            config,
...
impl<T: RpcTransport> RoochBridge<T> {
    pub fn with_transport(config: RoochConfig, transport: T) -> Self {
        Self {
            config,
            transport,
            next_id: Cell::new(1),
    ...
    pub fn config(&self) -> &RoochConfig {
        &self.config
    }
    ...
    /// Get an object's state from Rooch's global state tree.
